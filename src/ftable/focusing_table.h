...
typedef struct focusing_table {
	...
	float afterSlideRatio; // filled/capacity ratio that will make table to slide
	float invalidatedSlideThresholdRatio; // ratio of invalidated entries that will make table to slide

	unsigned int focusingHeadAddr; // current head, where headIndex points to the entry of this value
	unsigned int mappingUnit; // unit of mapping for an entry
} FTable;

FTable *ftable_create(unsigned int focusingHeadAddr);
int ftable_insert(FTable *ftable, unsigned int logicalSliceAddr, unsigned int virtualSliceAddr);
