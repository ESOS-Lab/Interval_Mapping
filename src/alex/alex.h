...
public:
	Alex() {
		...
		auto empty_data_node =
				new (data_node_allocator().allocate(1)) data_node_type(
	...
	// recommend directly using bulk_load() instead.
	template<class InputIterator>
	explicit Alex(InputIterator first, InputIterator last, const Alloc& alloc =
			Alloc()) :
			allocator_(alloc) {
		std::vector<V> values;
		for (auto it = first; it != last; ++it) {
	...
	explicit Alex(const self_type& other) :
			params_(other.params_), derived_params_(other.derived_params_), stats_(
					other.stats_), experimental_params_(
					other.experimental_params_), istats_(other.istats_), key_less_(
					other.key_less_), allocator_(other.allocator_) {
		superroot_ = static_cast<model_node_type*>(copy_tree_recursive(
				other.superroot_));
...
private:
	...
	AlexNode<T, P>* copy_tree_recursive(const AlexNode<T, P>* node) {
		if (!node)
			return nullptr;
		if (node->is_leaf_) {
			return new (data_node_allocator().allocate(1)) data_node_type(
					*static_cast<const data_node_type*>(node));
		} else {
			auto node_copy =
					new (model_node_allocator().allocate(1)) model_node_type(
							*static_cast<const model_node_type*>(node));
			int cur = 0;
			while (cur < node_copy->num_children_) {
				AlexNode<T, P>* child_node = node_copy->children_[cur];
				AlexNode<T, P>* child_node_copy = copy_tree_recursive(
						child_node);
...
// node's parent.
#if ALEX_SAFE_LOOKUP
	forceinline data_node_type* get_leaf(T key,
			std::vector<TraversalNode>* traversal_path = nullptr) const {
		if (traversal_path) {
			...
			if (cur->is_leaf_) {
				...
				if (std::abs(bucketID_prediction - bucketID_prediction_rounded)
					...
					if (bucketID_prediction_rounded <= bucketID_prediction) {
						if (leaf->prev_leaf_
								&& leaf->prev_leaf_->last_key() >= key) {
							if (traversal_path) {
								// Correct the traversal path
								correct_traversal_path(leaf, *traversal_path,
										true);
							}
...
private:
	...
	inline void correct_traversal_path(data_node_type* leaf,
			std::vector<TraversalNode>& traversal_path, bool left) const {
		if (left) {
			...
			if (start_bucketID == 0) {
				...
				while (!cur->is_leaf_) {
					auto node = static_cast<model_node_type*>(cur);
					traversal_path.push_back(
							{ node, node->num_children_ - 1 });
			...
			if (end_bucketID == parent->num_children_) {
				...
				while (end_bucketID == parent->num_children_) {
					...
					tn = traversal_path.back();
					parent = tn.node;
					end_bucketID = tn.bucketID - (tn.bucketID % repeats)
							+ repeats;
				}
	...
	T get_min_key() const {
		return first_data_node()->first_key();
	}

	// Returns maximum key in the index
	T get_max_key() const {
		return last_data_node()->last_key();
	}
...
public:
	...
	void bulk_load(const V values[], int num_keys) {
		...
		root_node_ = new (model_node_allocator().allocate(1)) model_node_type(0,
				allocator_);
		...
		DataNodeStats stats;
		root_node_->cost_ = data_node_type::compute_expected_cost(values,
				num_keys, data_node_type::kInitDensity_,
		...
		if (root_node_->is_leaf_) {
			static_cast<data_node_type*>(root_node_)->expected_avg_exp_search_iterations_ =
...
private:
	...
	void create_superroot() {
		if (!root_node_)
			return;
		delete_node(superroot_);
		superroot_ = new (model_node_allocator().allocate(1)) model_node_type(
				static_cast<short>(root_node_->level_ - 1), allocator_);
		superroot_->num_children_ = 1;
		superroot_->children_ = new (pointer_allocator().allocate(1)) AlexNode<
				T, P>*[1];
	...
	void update_superroot_key_domain() {
		...
		superroot_->model_.a_ = 1.0
				/ (istats_.key_domain_max_ - istats_.key_domain_min_);
		superroot_->model_.b_ = -1.0 * istats_.key_domain_min_
	...
	void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
		...
		if (best_fanout_tree_cost < node->cost_
			...
			if (best_fanout_tree_depth == 0) {
				...
				// in expectation
				best_fanout_tree_depth = static_cast<int>(std::log2(
						static_cast<double>(num_keys)
								/ derived_params_.max_data_node_slots)) + 1;
				used_fanout_tree_nodes.clear();
				fanout_tree::compute_level<T, P>(values, num_keys, node,
						total_keys, used_fanout_tree_nodes,
			...
			for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
				auto child_node =
						new (model_node_allocator().allocate(1)) model_node_type(
								static_cast<short>(node->level_ + 1),
								allocator_);
				child_node->cost_ = tree_node.cost;
				child_node->duplication_factor_ =
						static_cast<uint8_t>(best_fanout_tree_depth
								- tree_node.level);
				int repeats = 1 << child_node->duplication_factor_;
				double left_value = static_cast<double>(cur) / fanout;
				double right_value = static_cast<double>(cur + repeats)
						/ fanout;
				double left_boundary = (left_value - node->model_.b_)
						/ node->model_.a_;
				double right_boundary = (right_value - node->model_.b_)
						/ node->model_.a_;
				...
				if (model_node->children_[cur]->is_leaf_) {
					static_cast<data_node_type*>(model_node->children_[cur])->expected_avg_exp_search_iterations_ =
							tree_node.expected_avg_search_iterations;
					static_cast<data_node_type*>(model_node->children_[cur])->expected_avg_shifts_ =
	...
	data_node_type* bulk_load_leaf_node_from_existing(
			...
			bool compute_cost = true, const fanout_tree::FTNode* tree_node =
					nullptr, bool reuse_model = false, bool keep_left = false,
			bool keep_right = false) {
		auto node = new (data_node_allocator().allocate(1)) data_node_type(
				key_less_, allocator_);
		stats_.num_data_nodes++;
		if (tree_node) {
			...
			node->bulk_load_from_existing(existing_node, left, right, keep_left,
					keep_right, &precomputed_model, tree_node->num_keys);
		} else if (reuse_model) {
			...
			precomputed_model.expand(
					static_cast<double>(num_actual_keys) / (right - left));
			node->bulk_load_from_existing(existing_node, left, right, keep_left,
					keep_right, &precomputed_model, num_actual_keys);
		} else {
			node->bulk_load_from_existing(existing_node, left, right, keep_left,
					keep_right);
		}
		...
		if (compute_cost) {
			node->cost_ = node->compute_expected_cost(
					existing_node->frac_inserts());
		}
		return node;
	}
...
//				auto start_time = std::chrono::high_resolution_clock::now();
				...
				if (experimental_params_.splitting_policy_method == 0
						|| fail >= 2) {
					...
					// 2
					fanout_tree_depth =
							fanout_tree::find_best_fanout_existing_node<T, P>(
									parent, bucketID, stats_.num_keys,
									used_fanout_tree_nodes, 2);
...
//										- start_time).count();
				...
				if (fanout_tree_depth == 0) {
					...
					if (experimental_params_.allow_splitting_upwards) {
						...
						assert(
								experimental_params_.splitting_policy_method
										!= 2);
						int stop_propagation_level = best_split_propagation(
								traversal_path);
						if (stop_propagation_level <= superroot_->level_) {
							parent = split_downwards(parent, bucketID,
									fanout_tree_depth, used_fanout_tree_nodes,
									reuse_model);
						} else {
							split_upwards(key, stop_propagation_level,
									traversal_path, reuse_model, &parent);
						}
					} else {
						// either split sideways or downwards
						bool should_split_downwards = (parent->num_children_
								* best_fanout / (1 << leaf->duplication_factor_)
								> derived_params_.max_fanout
...
private:
	...
	int best_split_propagation(const std::vector<TraversalNode>& traversal_path,
		...
		if (verbose) {
			std::cout << "[Best split propagation] best level: "
					<< best_path_level << ", parent level: "
					<< traversal_path.back().node->level_ << ", best cost: "
					<< best_cost
	...
	void expand_root(T key, bool expand_left) {
		...
		if (expand_left) {
			...
			expansion_factor = pow_2_round_up(
					static_cast<int>(std::ceil(
							(key_difference + domain_size) / domain_size)));
			...
			T half_expandable_domain = istats_.key_domain_max_ / 2
			...
			auto key_difference = static_cast<double>(std::max(key,
					get_max_key()) - istats_.key_domain_max_);
			expansion_factor = pow_2_round_up(
					static_cast<int>(std::ceil(
							(key_difference + domain_size) / domain_size)));
			// Check for overflow. To avoid overflow on signed types while doing
			// this check, we do comparisons using half of the relevant quantities.
			T half_expandable_domain = std::numeric_limits<T>::max() / 2
					- istats_.key_domain_min_ / 2;
			T half_expanded_domain_size = expansion_factor / 2 * domain_size;
			if (half_expandable_domain < half_expanded_domain_size) {
		...
		if (root->num_children_ * expansion_factor
				<= derived_params_.max_fanout) {
			// Expand root node
			...
			auto new_children = new (
					pointer_allocator().allocate(new_num_children)) AlexNode<T,
					P>*[new_num_children];
			int copy_start;
			if (expand_left) {
			...
			pointer_allocator().deallocate(root->children_,
					root->num_children_);
			...
			auto new_root =
					new (model_node_allocator().allocate(1)) model_node_type(
							static_cast<short>(root->level_ - 1), allocator_);
			...
			new_root->children_ = new (
					pointer_allocator().allocate(expansion_factor)) AlexNode<T,
					P>*[expansion_factor];
			if (expand_left) {
		...
		if (expand_left) {
			in_bounds_new_nodes_start = std::max(new_nodes_start,
					root->model_.predict(new_domain_min));
		} else {
			in_bounds_new_nodes_end = std::min(new_nodes_end,
					root->model_.predict(new_domain_max) + 1);
		}
		...
		if (expand_left) {
			...
			int left_boundary = outermost_node->lower_bound(
					left_boundary_value);
			...
			for (int i = new_nodes_end; i > new_nodes_start; i -= n) {
				...
				if (i - n <= in_bounds_new_nodes_start) {
					...
					left_boundary = outermost_node->lower_bound(
							left_boundary_value);
				}
			...
			int right_boundary = outermost_node->lower_bound(
			...
			for (int i = new_nodes_start; i < new_nodes_end; i += n) {
				...
				if (i + n >= in_bounds_new_nodes_end) {
					...
					right_boundary = outermost_node->lower_bound(
							right_boundary_value);
				}
				data_node_type* new_node = bulk_load_leaf_node_from_existing(
						outermost_node, left_boundary, right_boundary, true);
		...
		// node.
		if (expand_left) {
			outermost_node->erase_range(new_domain_min,
					istats_.key_domain_min_);
			auto last_new_leaf =
					static_cast<data_node_type*>(root->children_[new_nodes_end
							- 1]);
	...
	model_node_type* split_downwards(model_node_type* parent, int bucketID,
		...
		double right_boundary_value = (end_bucketID - parent->model_.b_)
				/ parent->model_.a_;
		new_node->model_.a_ = 1.0 / (right_boundary_value - left_boundary_value)
				* fanout;
	...
	void split_sideways(model_node_type* parent, int bucketID,
		...
		if (fanout > repeats) {
			...
			int expansion_factor = parent->expand(
		...
		if (used_fanout_tree_nodes.empty()) {
			...
			int extra_duplication_factor = std::max(0,
					leaf->duplication_factor_ - fanout_tree_depth);
	...
	void create_two_new_data_nodes(data_node_type* old_node,
			model_node_type* parent, int duplication_factor, bool reuse_model,
		...
		data_node_type* left_leaf = bulk_load_leaf_node_from_existing(old_node,
				0, right_boundary, true, nullptr, reuse_model,
				append_mostly_right
						&& start_bucketID <= appending_right_bucketID
		...
		data_node_type* right_leaf = bulk_load_leaf_node_from_existing(old_node,
				right_boundary, old_node->data_capacity_, true, nullptr,
				reuse_model,
		...
		left_leaf->duplication_factor_ = static_cast<uint8_t>(duplication_factor
				- 1);
	...
	void create_new_data_nodes(const data_node_type* old_node,
			model_node_type* parent, int fanout_tree_depth,
			const std::vector<fanout_tree::FTNode>& used_fanout_tree_nodes,
	...
	data_node_type* split_upwards(T key, int stop_propagation_level,
		...
		auto leaf =
				static_cast<data_node_type*>(parent->children_[parent_path_node.bucketID]);
		int leaf_repeats = 1 << (leaf->duplication_factor_);
		int leaf_start_bucketID = parent_path_node.bucketID
				- (parent_path_node.bucketID % leaf_repeats);
		...
		bool left_half_appending_right = false, right_half_appending_right =
				false;
		if (append_mostly_right) {
			double appending_right_bucketID = parent->model_.predict_double(
					leaf->max_key_);
		...
		bool left_half_appending_left = false,
				right_half_appending_left = false;
		if (append_mostly_left) {
			double appending_left_bucketID = parent->model_.predict_double(
					leaf->min_key_);
		...
		data_node_type* left_leaf = bulk_load_leaf_node_from_existing(leaf, 0,
				mid_boundary, true, nullptr, reuse_model,
				append_mostly_right && left_half_appending_right,
				append_mostly_left && left_half_appending_left);
		data_node_type* right_leaf = bulk_load_leaf_node_from_existing(leaf,
				mid_boundary, leaf->data_capacity_, true, nullptr, reuse_model,
		...
		if (verbose) {
			std::cout << "[Splitting upwards data node] level " << leaf->level_
					<< ", node addr: " << leaf << ", node repeats in parent: "
					<< leaf_repeats << ", node indexes in parent: ["
					<< leaf_start_bucketID << ", " << leaf_end_bucketID << ")"
					<< ", left leaf indexes: [0, " << mid_boundary << ")"
					<< ", right leaf indexes: [" << mid_boundary << ", "
					<< leaf->data_capacity_ << ")" << ", new nodes addr: "
					<< left_leaf << "," << right_leaf << std::endl;
		}
		...
		while (traversal_path[path_idx].node->level_ > stop_propagation_level) {
			...
			bool double_left_half = path_node.bucketID
			...
			if (double_left_half) {
				...
				left_split->children_ =
						new (
								pointer_allocator().allocate(
										left_split->num_children_)) AlexNode<T,
								P>*[left_split->num_children_];
				...
				while (cur < cur_node->num_children_ / 2) {
					...
					for (int i = 2 * cur; i < 2 * (cur + cur_child_repeats);
							i++) {
				...
				if (pull_up_right_child) {
					...
					right_split->children_ =
							new (
									pointer_allocator().allocate(
											right_split->num_children_)) AlexNode<
									T, P>*[right_split->num_children_];
					right_split->model_.a_ = cur_node->model_.a_;
					right_split->model_.b_ = cur_node->model_.b_
							- cur_node->num_children_ / 2;
					int j = 0;
					for (int i = cur_node->num_children_ / 2;
							i < cur_node->num_children_; i++) {
				...
				if (pull_up_left_child) {
					...
					left_split->children_ =
							new (
									pointer_allocator().allocate(
											left_split->num_children_)) AlexNode<
									T, P>*[left_split->num_children_];
				...
				right_split->children_ =
						new (
								pointer_allocator().allocate(
										right_split->num_children_)) AlexNode<T,
								P>*[right_split->num_children_];
				right_split->model_.a_ = cur_node->model_.a_ * 2;
				right_split->model_.b_ = (cur_node->model_.b_
						- cur_node->num_children_ / 2) * 2;
				...
				while (cur < cur_node->num_children_) {
					...
					for (int i = 2 * right_child_idx;
							i < 2 * (right_child_idx + cur_child_repeats);
							i++) {
				...
				int new_bucketID = (path_node.bucketID
						- cur_node->num_children_ / 2) * 2;
				int repeats = 1 << (prev_left_split->duplication_factor_ + 1);
				int start_bucketID = new_bucketID - (new_bucketID % repeats); // first bucket with same child
				int mid_bucketID = start_bucketID + repeats / 2;
			...
			if (verbose) {
				std::cout << "[Splitting upwards through-node] level "
						...
						<< ", new nodes addr: " << left_split << ","
						<< right_split << std::endl;
			}
			...
			// This is the expected duplication factor; it will be correct once we
			// split/expand the parent
			next_left_split->duplication_factor_ =
					cur_node->duplication_factor_;
			next_right_split->duplication_factor_ =
					cur_node->duplication_factor_;
		...
		int repeats = 1 << prev_left_split->duplication_factor_; // this was the duplication
		...
		while (path_pos >= 0) {
			...
			while (leaf->num_keys_ == 0 && repeats < parent->num_children_) {
				...
				if (adjacent_to_right
					...
					adjacent_leaf =
							static_cast<data_node_type*>(parent->children_[start_bucketID
									- 1]);
				} else {
			...
			if (!cur->is_leaf_) {
				...
				if (!node->validate_structure(true)) {
					std::cout << "[Model node invalid structure]"
							<< " node addr: " << node << ", node level: "
							<< node->level_ << std::endl;
					if (short_circuit) {
						return false;
					} else {
						is_valid = false;
				...
				if (validate_data_nodes) {
					...
					if (node->num_keys_ > 0) {
						...
						if (next_nonempty_leaf) {
							T first_in_next_leaf =
									next_nonempty_leaf->first_key();
...
public:
	...
	public:
		...
		Iterator(data_node_type* leaf, int idx) :
				cur_leaf_(leaf), cur_idx_(idx) {
			initialize();
		}

		Iterator(const Iterator& other) :
				cur_leaf_(other.cur_leaf_), cur_idx_(other.cur_idx_), cur_bitmap_idx_(
						other.cur_bitmap_idx_), cur_bitmap_data_(
						other.cur_bitmap_data_) {
		}
